...
function styleLineRecurse(syntax, line, edits){
    if (typeof syntax.match === 'string'){
        var matcher = RegExp(syntax.match, 'g');
        var matchResult = matcher.exec(line);
        while (matchResult){
            var color = colorConversion(syntax.name);
            if (color){
                edits.push({offset: matchResult.index, text: '<span style="color: ' + color + '">', name: syntax.name});
                edits.push({offset: matchResult.index + matchResult[0].length, text: '</span>', name: syntax.name});
            }
            matchResult = matcher.exec(line);
        }
